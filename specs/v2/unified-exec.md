...
Linux unified exec always selects Bash from OpenCode's PATH, independently of `config.shell` and `$SHELL`, with no fallback to sh. Both transports use non-interactive `bash --noprofile --norc -p`: pipe lanes use `-s` (pipe stdin may be a socket that cannot be reopened), and TTY lanes use `/dev/stdin` as the script operand. The host rejects unequal real/effective user or group IDs before this startup. Bash itself ignores inherited option/function state, `CDPATH`, `GLOBIGNORE`, and startup scripts in this mode. The prepared OpenCode/plugin environment is retained with `BASH_ENV`, `ENV`, `HISTFILE`, and `BASH_COMPAT` explicitly empty, and bootstrap executes `set +o posix` before sourcing the runner. TTY transport does not enable interactive-parent prompt hooks, history, or job control, but child REPLs still receive a terminal.

These defaults apply at lane creation, not before every command. Explicit `source` and user shell options remain persistent. The POSIX runner writes its private cwd status file with forced redirection on non-Windows hosts so `noclobber` cannot corrupt completion reporting; normal user redirections retain their chosen semantics. Ctrl+C can end the generation and permit rebuilding, while explicit exit or disabling shell execution may lose state. Other interpreters must be invoked as nested commands. Integrated terminals and other shell consumers are unchanged.

On other platforms the runtime retains its explicit adapters for PowerShell 7, Windows PowerShell 5.1, cmd, and recognized POSIX shells (`bash`, `dash`, `ksh`, `sh`, and `zsh`, including recognized Git Bash paths on Windows). Unsupported shell adapters fail with `unsupported persistent shell protocol`.
