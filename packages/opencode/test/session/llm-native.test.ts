...
import { LLMEvent, ToolFailure, type LLMRequest } from "@opencode-ai/llm"
...
describe("session.llm-native.request", () => {
  ...
  it.effect("native compact delegates OpenAI GPT requests to LLMClient.compactWithInput", () =>
    Effect.gen(function* () {
      let captured: LLMRequest | undefined
      const llmClient = {
        prepare: () => Effect.die("unused"),
        compact: () => Effect.die("unused"),
        compactWithInput: (request: LLMRequest) =>
          Effect.sync(() => {
            captured = request
            return { output: [{ type: "compaction", encrypted_content: "opaque" }], input: [] }
          }),
        stream: () => Stream.empty,
      ...
      const result = yield* LLMNativeRuntime.compact({
        ...
        auth: undefined,
        llmClient,
        messages: [{ role: "system", content: "legacy summary prompt" }, storedSession.user("compact this")],
        providerOptions: { instructions: "legacy summary prompt", promptCacheKey: "cache-key" },
        headers: {},
      })
      ...
      expect(captured?.model.route.id).toBe("openai-responses")
      expect(captured?.messages).toMatchObject([{ role: "user" }])
      expect(captured?.system).toHaveLength(0)
      expect(captured?.providerOptions).toMatchObject({ openai: { promptCacheKey: "cache-key" } })
      expect(JSON.stringify(captured?.providerOptions)).not.toContain("legacy summary prompt")
    }),
  )

  it.effect("native compact supports official non-GPT OpenAI models", () =>
    Effect.gen(function* () {
      let captured: LLMRequest | undefined
      const llmClient = {
        prepare: () => Effect.die("unused"),
      ...
      const result = yield* LLMNativeRuntime.compact({
        model: { ...baseModel, id: ModelV2.ID.make("o4-mini"), api: { ...baseModel.api, id: "o4-mini" } },
        provider: providerInfo,
        auth: undefined,
        llmClient,
        messages: [storedSession.user("compact this")],
        headers: {},
      })

      expect(result.type).toBe("supported")
      if (result.type === "unsupported") throw new Error(result.reason)
      expect(String(captured?.model.id)).toBe("o4-mini")
    }),
  )
  ...
  it.effect("uses provider fetch override for native OpenAI OAuth compact requests", () =>
    Effect.gen(function* () {
      const captures: Array<{ url: string; body: unknown }> = []
      const customFetch = Object.assign(
        async (input: Parameters<typeof fetch>[0], init: Parameters<typeof fetch>[1]) => {
          ...
          return new Response(
            JSON.stringify({
              output: [{ type: "compaction", encrypted_content: "opaque", id: "cmp_1" }],
            }),
      ...
      expect(result.output).toEqual([{ type: "compaction", encrypted_content: "opaque", id: "cmp_1" }])
    }),
  )

  it.effect("native compact includes prior OpenAI compaction window before the trigger", () =>
    Effect.gen(function* () {
      const captures: Array<{ url: string; body: unknown }> = []
      const customFetch = Object.assign(
        async (input: Parameters<typeof fetch>[0], init: Parameters<typeof fetch>[1]) => {
          ...
          return new Response(
            JSON.stringify({
              output: [{ type: "compaction", encrypted_content: "next-window" }],
            }),
            { headers: { "content-type": "application/json" } },
          )
        },
      ...
      const result = yield* LLMNativeRuntime.compact({
        ...
        auth: { type: "oauth", refresh: "refresh", access: "access", expires: Date.now() + 60_000 },
        llmClient,
        messages: [{ role: "user", content: "after checkpoint" }],
        headers: {},
        nativeCompactionWindow: {
          version: 1,
          output: [{ type: "compaction", encrypted_content: "previous-window", id: "cmp_previous" }],
        },
      })
      ...
      expect(captures[0]).toMatchObject({
        ...
        body: {
          ...
          stream: true,
          input: [
            { type: "compaction", encrypted_content: "previous-window", id: "cmp_previous" },
            { role: "user", content: [{ type: "input_text", text: "after checkpoint" }] },
            { type: "compaction_trigger" },
          ],
        },
      })
      expect(result.output).toEqual([{ type: "compaction", encrypted_content: "next-window" }])
    }),
  )

  it.effect("replays OpenAI turn state across stream and compact requests", () =>
    Effect.gen(function* () {
      const requests: LLMRequest[] = []
      const turnState = LLMNativeRuntime.createTurnState()
      const llmClient: LLMClientShape = {
        prepare: () => Effect.die("unused prepare"),
        compact: () => Effect.die("unused compact"),
        generate: () => Effect.die("unused generate"),
        stream: (request) => {
      ...
      const first = LLMNativeRuntime.stream({
        model: baseModel,
        provider: providerInfo,
        auth: undefined,
        llmClient,
      ...
      expect(first.type).toBe("supported")
      if (first.type === "unsupported") throw new Error(first.reason)
      yield* first.stream.pipe(Stream.runCollect)
      ...
      expect(result.type).toBe("supported")
      if (result.type === "unsupported") throw new Error(result.reason)
      expect(turnState.value).toBe("turn-1")
      expect(requests).toHaveLength(2)
      expect(requests[1].model.route.defaults.headers).toMatchObject({ "x-codex-turn-state": "turn-1" })
      expect(requests[1].providerOptions?.openai?.clientMetadata).toBeUndefined()
      expect(result.providerMetadata).toEqual({ openai: { headers: { "x-codex-turn-state": "compact-turn" } } })
    }),
  )
})
