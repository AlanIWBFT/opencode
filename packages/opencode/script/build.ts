...
const windowsGuiSubsystem = process.argv.includes("--windows-gui-subsystem")
...
const targets = selectBuildTargets(allTargets, process.argv.slice(2), {
  os: process.platform,
  arch: process.arch,
})
if (windowsGuiSubsystem && targets.some((item) => item.os !== "win32")) {
  throw new Error("--windows-gui-subsystem requires Windows build targets")
}
...
if (!skipInstall) {
  await $`bun install --no-save --frozen-lockfile --os="*" --cpu="*" @opentui/core@${pkg.dependencies["@opentui/core"]}`
  await $`bun install --no-save --frozen-lockfile --os="*" --cpu="*" @parcel/watcher@${pkg.dependencies["@parcel/watcher"]}`
  await $`bun install --no-save --frozen-lockfile --os="*" --cpu="*" @ff-labs/fff-bun@${pkg.dependencies["@ff-labs/fff-bun"]}`
}
for (const item of targets) {
  ...
  await Bun.build({
    ...
    compile: {
      ...
      windows: windowsGuiSubsystem ? { hideConsole: true } : {},
    },
    files: {
